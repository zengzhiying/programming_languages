        ...
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            <div class="am-btn-group am-btn-group-xs">
              <button type="button" class="am-btn am-btn-default" onclick="location.href='<%=pagePath %>/webadmin/partment/add'"><span class="am-icon-plus"></span> 新增部门</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="am-u-sm-12">
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              <thead>
              <tr>
                <th class="">部门编号</th><th class="table-title">部门名称</th><th class="table-type">部门简介</th><th class="table-author am-hide-sm-only">成立时间</th><th class="table-set">操作</th>
              </tr>
              </thead>
              <tbody>
              <%
              	List<Object> pmAll = (List<Object>) request.getAttribute("pmAll");
              	if(pmAll == null ||pmAll.size() == 0) {
              		%>
              		<p style="font-size:22px;">暂时没有部门！</p>
              		<%
              	} else {
              		for(Object partment:pmAll) {
              			Partment pm = (Partment) partment;
              		%>
              		<tr>
		                <td><%=pm.getPm_id() %></td>
		                <td><a href="#"><%=pm.getPm_name() %></a></td>
		                <td><%
		                if(pm.getPm_describe().equals("0")) {
		                	out.print("暂无");
		                } else {
		                	out.print(pm.getPm_describe());
		                }
							%></td>
		                <td class="am-hide-sm-only"><%
		                //注册时间转换
		                DataTypeConver dtc = new DataTypeConver();
		                out.print(dtc.timeToString(pm.getSetup_time(),"yyyy-MM-dd"));
		                %></td>
		                <td>
		                  <div class="am-btn-toolbar">
		                    <div class="am-btn-group am-btn-group-xs">
		  
		                      <a class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="location.href='<%=pagePath %>/webadmin/partment/edit?partment_id=<%=pm.getPm_id() %>'"><span class="am-icon-pencil-square-o"></span> 编辑详情</a>
		                      <a class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" href="javascript:;" onclick="delete_partment(<%=pm.getPm_id() %>);"><span class="am-icon-trash-o"> 删除</span></a>
...
function delete_partment(pm_id) {
