...
<!-- content start -->
  <div class="admin-content">
    <div class="admin-content-body" style="margin-left:36px;">
      <div class="am-cf am-padding am-padding-bottom-0">
        <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">部门经理编辑界面</strong> / <small>edit</small></div>
      </div>
      <hr>
  ...
  <%
   User u = (User) request.getAttribute("user");
      ...
      <label for="doc-ipt-email-1">经理姓名</label>
      <input type="text" class="" value="<%=u.getUser_name() %>" name="user_name" id="user_name" placeholder="请输入真实姓名">
    </div>
    ...
    <div class="am-form-group">
      <label for="doc-select-1">请填写年龄</label>
      <input type="text" class="" name="age" id="age" value="<%=u.getAge() %>" placeholder="请输入年龄">
    </div>
    ...
    <div class="am-form-group">
      <label for="doc-ta-1">请填写经理简介</label>
      <textarea name="user_describe" id="user_describe" class="" rows="5" id="doc-ta-1" placeholder="介绍一下吧"><%
    ...
    <div class="am-form-group">
      <label for="doc-select-1">请设置登录用户名</label>
      <input type="text" class="" value="<%=u.getUsername() %>" name="username" id="username" placeholder="请设置用户名">
    </div>
    
    <div class="am-form-group">
      <label for="doc-select-1">请设置新密码</label>
      <input type="password" class="" value="" name="password" id="password" placeholder="不修改请留空">
    </div>
  ...
  %>
    <div class="am-form-group">
      <label for="doc-ipt-email-1">经理姓名</label>
      <input type="text" class="" name="user_name" id="user_name" placeholder="请输入真实姓名">
    </div>
    ...
    <div class="am-form-group">
      <label for="doc-select-1">请填写年龄</label>
      <input type="text" class="" name="age" id="age" placeholder="请输入年龄">
    </div>
    ...
    <div class="am-form-group">
      <label for="doc-ta-1">请填写简介</label>
      <textarea name="user_describe" id="user_describe" class="" rows="5" id="doc-ta-1" placeholder="介绍一下吧"></textarea>
    </div>
    <div class="am-form-group">
      <label for="doc-select-1">请设置登录用户名</label>
      <input type="text" class="" value="" name="username" id="username" placeholder="请设置用户名">
    </div>
    
    <div class="am-form-group">
      <label for="doc-select-1">请设置新密码</label>
      <input type="password" class="" value="" name="password" id="password" placeholder="请设置新密码">
    </div>
		...
		*/
		if(typeof user_id === 'undefined' || user_id == "") {
			...
			$.ajax({
	            type:"post",
	            async:true,
	            url:"<%=pagePath %>/webadmin/partment_manage/add",
	            //dataType:"html",
	            data:{"user_name":user_name,"gender":gender,"age":age,"partment_id":partment_id,"contact":contact,"user_describe":user_describe,"username":username,"password":password},
	            success:function(data) {
	              ...
	              if(data == "success") {
	            	  //layer.msg("新增员工成功！");
	            	  layer.alert('新增部门经理成功！', {
						  skin: 'layui-layer-lan' //样式类名
						  ,closeBtn: 0
						}, function(){
						  location.href="<%=request.getContextPath() %>/webadmin/partment_manager";
						});
	            	  return true;
	              }
	              if(data == "error") {
			...
			$.ajax({
	            ...
	            //dataType:"html",
	            data:{"user_name":user_name,"gender":gender,"age":age,"partment_id":partment_id,"contact":contact,"user_describe":user_describe,"user_id":user_id,"action":action,"username":username,"password":password},
	            success:function(data) {
	            	//请求成功 关闭loding
		            layer.closeAll('loading');
	              if(data == "empty") {
